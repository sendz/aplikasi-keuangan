...
class Model_pemasukan extends CI_Model {
  ...
  public function perSiswa($by,$id) {
    $query = $this->db->query("SELECT GROUP_CONCAT(pemasukan.id) AS id, siswa.id AS idsiswa, siswa.nis AS nis, siswa.nama AS namasiswa, GROUP_CONCAT(pemasukan.tanggal) AS tanggal, GROUP_CONCAT(pemasukan.jumlah) AS jumlah, kelas.tingkat AS tingkat, kelas.nama AS namakelas, biayapendidikan.jumlah AS biayapendidikan FROM siswa, pemasukan, kelas, biayapendidikan WHERE (pemasukan.idsiswa = siswa.id AND kelas.id = siswa.idkelas) AND biayapendidikan.tingkat = kelas.tingkat AND pemasukan.keterangan = 'pemasukan' AND siswa.$by = $id GROUP BY siswa.id ORDER BY siswa.nama");
    return $query->result();
  }
  ...
  public function listPemasukan() {
    ...
    $query = $this->db->query("SELECT GROUP_CONCAT(pemasukan.id) AS id, siswa.id AS idsiswa, siswa.nis AS nis, siswa.nama AS namasiswa, GROUP_CONCAT(pemasukan.tanggal) AS tanggal, GROUP_CONCAT(pemasukan.jumlah) AS jumlah, kelas.tingkat AS tingkat, kelas.nama AS namakelas, biayapendidikan.jumlah AS biayapendidikan FROM siswa, pemasukan, kelas, biayapendidikan WHERE (pemasukan.idsiswa = siswa.id AND kelas.id = siswa.idkelas) AND biayapendidikan.tingkat = kelas.tingkat AND pemasukan.keterangan = 'pemasukan' GROUP BY siswa.id ORDER BY siswa.nama");
    return $query->result();
  }
